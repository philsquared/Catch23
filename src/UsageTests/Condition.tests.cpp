...
TEST_CASE( "Equality checks that should succeed" )
{
    ...
    REQUIRE_THAT( data.float_nine_point_one, is_close_to( 9.1f ) );
    REQUIRE_THAT( data.double_pi, is_close_to( 3.1415926535 ) );
...
TEST_CASE( "Equality checks that should succeed - macro-free" )
{
    ...
    require() << data.int_seven == 7; // NOSONAR NOLINT (misc-typo)
    require().that( data.float_nine_point_one, is_close_to( 9.1f ) );
    require().that( data.double_pi, is_close_to( 3.1415926535 ) );
    require() << data.str_hello == "hello"; // NOSONAR NOLINT (misc-typo)
    require() << "hello" == data.str_hello; // NOSONAR NOLINT (misc-typo)
    require() << data.str_hello.size() == 5; // NOTSONAR NOLINT (misc-typo)
...
// Needed to test junit reporter's handling of may_fail test cases and sections
...
TEST_CASE( "Inequality checks that should fail", "[.][failing][!mayfail]" )
{
    ...
    CHECK_THAT( data.float_nine_point_one, !is_close_to( 9.1f ) );
...
// the equals(false) matcher can be used instead to capture the operand value.
// (also works with the convenience matcher, is_false())
